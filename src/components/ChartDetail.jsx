...
const CustomLabel = (props) => {
  const { x, y, width, value, isHovered } = props;
  const labelWidth = 55;
  const labelHeight = 30;

  // Only show the label if the bar is hovered
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        <Bar dataKey="amount" radius={[5, 5, 5, 5]} onClick={handleClick}>
          <LabelList
            dataKey="amount"
            content={(props) => (
              <CustomLabel
            ...
            <Cell
              ...
              fill={
                hoveredIndex === index
                  ? index === activeDay
                    ? "#B4DFE5"
                    : "#FF9B87" // Hover state based on activeDay
                  : index === activeDay
                    ? "hsl(186, 34%, 60%)"
                    : "hsl(10, 79%, 65%)" // Default fill
              }
